...
export function ChatMessage({ role, content, suggestions, timestamp, onTypeUpdate, eventContext: _unusedProp }) {
  ...
  // ── Key fix: pass event_type as a query param so the backend uses
  //    dress-code-aware pairing rules (no T-shirts with pencil skirts, etc.)
  const handleFindPair = async (item) => {
    try {
      // item.eventContext is attached in Chat.jsx when building suggestions
      const eventContext = item.eventContext || null;
      const url = eventContext
        ? `http://localhost:5000/api/outfit-pairing/${item.id}?event_type=${encodeURIComponent(eventContext)}`
        : `http://localhost:5000/api/outfit-pairing/${item.id}`;
  ...
  const handleMarkPairAsWorn = async (pairItem) => {
    if (!pairingResults || !window.confirm(
      `Mark ${pairingResults.selectedItem.type} + ${pairItem.type} as worn together?`
    )) return;
