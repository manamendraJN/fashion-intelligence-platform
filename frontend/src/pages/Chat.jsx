...
import { Send, Sparkles, User, ChevronRight, Sun, CloudRain, Snowflake } from 'lucide-react';
import { motion } from 'framer-motion';

// ─── Event Categories ─────────────────────────────────────────────────────────
// `value`  = EXACT string sent to /api/recommend-smart as "occasion"
//            Must match STANDARD_EVENTS in event_constants.py exactly.
// `query`  = human-readable text shown in the user message bubble
// `icon`   = emoji shown in sidebar button
const EVENT_CATEGORIES = [
  {
    label: 'Occasions',
    events: [
      { name: 'Tamil Wedding',    icon: '💍', value: 'Tamil Wedding',    query: 'Tamil Wedding' },
      { name: 'Western Wedding',  icon: '👰', value: 'Western Wedding',  query: 'Western Wedding' },
      { name: 'Party',            icon: '🎉', value: 'Party',            query: 'party night out' },
      { name: 'Date Night',       icon: '❤️',  value: 'Date',             query: 'date night dinner' },
      { name: 'Family Gathering', icon: '👨‍👩‍👧', value: 'Family Gathering', query: 'family gathering' },
      { name: 'Religious',        icon: '🙏', value: 'Religious',        query: 'religious event temple' },
      { name: 'Funeral',          icon: '🖤', value: 'Funeral',          query: 'funeral memorial' },
    ],
  },
  {
    label: 'Work & Daily',
    events: [
      { name: 'Office',          icon: '💼', value: 'Office',          query: 'office work day' },
      { name: 'Office Meeting',  icon: '🎯', value: 'Office Meeting',  query: 'important office meeting' },
      { name: 'Casual',          icon: '☀️',  value: 'Casual',          query: 'casual outing weekend' },
      { name: 'Shopping',        icon: '🛍️', value: 'Shopping',        query: 'shopping mall' },
      { name: 'Lunch',           icon: '🍽️', value: 'Casual',          query: 'casual lunch brunch' },
      { name: 'Travel',          icon: '✈️',  value: 'Casual',          query: 'travel casual trip' },
    ],
  },
  {
    label: 'Active & Sports',
    events: [
      { name: 'Gym',     icon: '💪', value: 'Gym',    query: 'gym workout fitness' },
      { name: 'Running', icon: '🏃', value: 'Sports', query: 'running jogging outdoor' },
      { name: 'Yoga',    icon: '🧘', value: 'Gym',    query: 'yoga pilates fitness' },
      { name: 'Beach',   icon: '🏖️', value: 'Beach',  query: 'beach pool swimming' },
      { name: 'Hiking',  icon: '🥾', value: 'Sports', query: 'hiking outdoor sports' },
      { name: 'Sports',  icon: '⚽', value: 'Sports', query: 'sports athletic game' },
    ],
  },
];

const WEATHER_OPTIONS = [
  { label: 'Hot',   value: 'hot',   icon: <Sun className="w-3.5 h-3.5" />,       color: 'bg-orange-100 text-orange-700 border-orange-200' },
  { label: 'Cold',  value: 'cold',  icon: <Snowflake className="w-3.5 h-3.5" />, color: 'bg-blue-100 text-blue-700 border-blue-200' },
  { label: 'Rainy', value: 'rainy', icon: <CloudRain className="w-3.5 h-3.5" />, color: 'bg-slate-100 text-slate-700 border-slate-200' },
];

// Display names for response headers
const EVENT_DISPLAY = {
  'Casual': 'Casual',
  'Office': 'Office',
...
export function ChatPage() {
  ...
  const [messages, setMessages] = useState([{
    ...
    content:
      "Hello! 👋 I've analysed your wardrobe and I'm ready to help.\n\n" +
      "Tell me what's on your calendar — pick an event below or type anything.\n" +
      "I'll suggest the best outfits, considering what you've worn recently.",
  }]);

  useEffect(() => {
    fetch('http://localhost:5000/api/wardrobe')
      .then(r => r.json()).then(setWardrobe).catch(console.error);
  }, []);
  ...
  // ── Free-text → { value, display } ──────────────────────────────────────
  function extractOccasion(text) {
    const t = text.toLowerCase();

    // Funeral (check before wedding/religious)
    if (t.includes('funeral') || t.includes('condolence') || t.includes('memorial') || t.includes('mourning'))
      return { value: 'Funeral', display: 'Funeral' };

    // Weddings
    if (t.includes('tamil wedding') || t.includes('indian wedding') || t.includes('traditional wedding'))
      return { value: 'Tamil Wedding', display: 'Tamil Wedding' };
    if (t.includes('western wedding') || t.includes('church wedding') ||
        (t.includes('wedding') && !t.includes('tamil') && !t.includes('indian')))
      return { value: 'Western Wedding', display: 'Western Wedding' };

    // Work
    if (t.includes('office meeting') || t.includes('business meeting') ||
        t.includes('important meeting') || t.includes('interview'))
      return { value: 'Office Meeting', display: 'Office Meeting' };
    if (t.includes('office') || t.includes('work') || t.includes('business') || t.includes('professional'))
      return { value: 'Office', display: 'Office' };

    // Social
    if (t.includes('party') || t.includes('club') || t.includes('night out'))
      return { value: 'Party', display: 'Party' };
    if (t.includes('date night') || t.includes('date') || t.includes('romantic') || t.includes('dinner'))
      return { value: 'Date', display: 'Date Night' };
    if (t.includes('family') || t.includes('reunion'))
      return { value: 'Family Gathering', display: 'Family Gathering' };
    if (t.includes('temple') || t.includes('church') || t.includes('mosque') || t.includes('religious'))
      return { value: 'Religious', display: 'Religious Event' };

    // Shopping / daily
    if (t.includes('shopping') || t.includes('mall'))
      return { value: 'Shopping', display: 'Shopping' };
    if (t.includes('lunch') || t.includes('brunch') || t.includes('travel'))
      return { value: 'Casual', display: 'Casual' };

    // Active
    if (t.includes('gym') || t.includes('workout') || t.includes('yoga') || t.includes('pilates'))
      return { value: 'Gym', display: 'Gym' };
    if (t.includes('running') || t.includes('jogging'))
      return { value: 'Sports', display: 'Running' };
    if (t.includes('beach') || t.includes('swim') || t.includes('pool'))
      return { value: 'Beach', display: 'Beach' };
    if (t.includes('hiking') || t.includes('outdoor') || t.includes('sports') || t.includes('athletic'))
      return { value: 'Sports', display: 'Sports' };

    // Casual fallback
    if (t.includes('casual') || t.includes('weekend') || t.includes('chill') || t.includes('relax') || t.includes('outing'))
      return { value: 'Casual', display: 'Casual' };
  ...
  function extractWeather(text) {
    ...
    return selectedWeather;   // fall back to sidebar selection
  }

  // ── Send message ────────────────────────────────────────────────────────
  // overrideValue: exact STANDARD_EVENTS string (from sidebar button)
  const handleSend = async (text, overrideValue = null) => {
    const msg = (text || input).trim();
    if (!msg) return;

    setMessages(prev => [...prev, {
      id: Date.now().toString(), role: 'user',
      content: msg, timestamp: new Date(), suggestions: [],
    }]);
    setInput('');
    setIsTyping(true);
    ...
    // Resolve occasion: sidebar click → use exact value, free text → parse
    let occasion = null;
    if (overrideValue) {
      occasion = { value: overrideValue, display: EVENT_DISPLAY[overrideValue] || overrideValue };
    } else {
      occasion = extractOccasion(msg);
    }
      ...
      try {
        ...
        if (data.success) {
          ...
          if (suggestions.length === 0) {
            responseContent =
              `😕 No items found in your wardrobe for **${occasion.display}**.\n\n` +
              `💡 Tip: Tap **Recalculate All Scores** in Settings to refresh your wardrobe data, ` +
              `or upload new items.`;
          } else {
            // Build response header
            const weatherEmoji = weather === 'cold' ? '❄️ Cold' : weather === 'hot' ? '☀️ Hot' : weather === 'rainy' ? '🌧️ Rainy' : '';
            const weatherLine  = weatherEmoji ? ` · ${weatherEmoji} weather` : '';

            if (occasion.value === 'Funeral') {
              responseContent = `🖤 Appropriate attire for **${occasion.display}**${weatherLine}:\n_(Dark & neutral colours only)_\n\n`;
            } else {
              responseContent = `✨ Best picks for **${occasion.display}**${weatherLine}:\n\n`;
            }

            if (data.recentlyWorn?.length > 0) {
              responseContent += `📝 Skipped ${data.recentlyWorn.length} recently worn item(s) for variety.\n\n`;
            }
          ...
          responseContent = `Couldn't fetch recommendations right now. Please try again!`;
        }
      } catch (err) {
        console.error('Recommend error:', err);
        responseContent = `Something went wrong. Please check your connection and try again.`;
      }
    } else {
      responseContent =
        "I couldn't identify a specific occasion from that message.\n\n" +
        "Try tapping an event button in the sidebar, or type something like:\n" +
        ...
        <div className="flex gap-6 h-[calc(100vh-96px)]">
          ...
          <aside className="hidden lg:flex flex-col w-64 flex-shrink-0 gap-4">
            ...
            <div className="bg-white rounded-2xl border border-[#EDE8E0] p-3">
              <p className="text-[10px] font-semibold text-[#A0998F] uppercase tracking-wider mb-2 px-1">
                Weather Filter
              </p>
              <div className="flex flex-col gap-1.5">
                {WEATHER_OPTIONS.map(w => (
                  <button
                    key={w.label}
                    onClick={() => setSelectedWeather(prev => prev === w.value ? null : w.value)}
                  ...
                  >
                    {w.icon} {w.label}
                    {selectedWeather === w.value && <span className="ml-auto text-[10px]">✓</span>}
                  </button>
                ))}
              </div>
            </div>
                  ...
                  <div className="flex flex-col gap-0.5">
                    {cat.events.map(ev => (
                      <button
                        key={ev.name}
                        onClick={() => handleSend(
                          `What should I wear for ${ev.query}?`,
                          ev.value
                        )}
                        disabled={isTyping}
                      ...
                      >
                        <span className="text-base leading-none w-5 text-center">{ev.icon}</span>
                        <span>{ev.name}</span>
            ...
            <div className="lg:hidden flex-shrink-0 px-4 py-2 border-t border-[#F0EBE3] overflow-x-auto scrollbar-hide">
              <div className="flex gap-2 pb-1">
                {EVENT_CATEGORIES.flatMap(c => c.events).map(ev => (
                  <button
                    key={ev.name}
                    onClick={() => handleSend(`What should I wear for ${ev.query}?`, ev.value)}
                    disabled={isTyping}
            ...
            <div className="flex-shrink-0 px-5 py-4 border-t border-[#F0EBE3] bg-[#FDFAF7]">
              {selectedWeather && (
                <div className="flex items-center gap-2 mb-3">
                  <span className="text-xs text-[#8B7A6A] bg-[#F5EFE8] px-3 py-1 rounded-full border border-[#E8DFD4]">
                    {selectedWeather === 'cold' ? '❄️' : selectedWeather === 'hot' ? '☀️' : '🌧️'}{' '}
                    Filtering for {selectedWeather} weather
                  </span>
              ...
              <div className="flex gap-3 items-end">
                <input
                  ...
                  className="flex-1 px-5 py-3.5 bg-[#F5F0EB] border border-transparent rounded-2xl focus:outline-none focus:border-[#8B5A5A] focus:bg-white transition-all text-[13px] text-[#2C2C2C] placeholder:text-[#B0A898] disabled:opacity-50"
                />
                <button
